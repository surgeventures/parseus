defmodule ParseusTest.SomeStruct do
  defstruct [:some_field]
end

defmodule ParseusTest.OtherStruct do
  defstruct [:other_field]
end

defmodule ParseusTest do
  use ExUnit.Case
  import Parseus
  alias Parseus.Error
  alias ParseusTest.{SomeStruct, OtherStruct}
  ...
  describe "parse_list/3" do
    ...
    test "with item parser" do
      assert {:ok, [1, 2]} =
        [ids: "1,2"]
        |> cast(:ids)
        |> parse_list(:ids, item_parser: Parseus.IntegerParser)
        |> resolve_tuple(:ids)
    end
